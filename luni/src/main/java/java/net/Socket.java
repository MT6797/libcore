...
public class Socket implements Closeable {
    ...
    public synchronized void close() throws IOException {
        ...
        try {
            System.out.println("close [socket][" + localAddress + ":" + getLocalPort() + "]");
        } catch (Exception e) {
        ...
        synchronized (connectLock) {
            try {
                ...
                System.out.println("[socket][" + sIndex + "] connection "
                        + remoteAddr + ";LocalPort=" + getLocalPort() + "(" + timeout + ")");
                sIndex = sIndex + 1;
                /// @}
                ...
                /// M: debug logging
                System.out.println("[socket][" + this.localAddress
    ...
    ///M: Add by Mediatek for features @{
    /** Cached instance of android.security.NetworkSecurityPolicy. */
    private static Object networkSecurityPolicy;
    /** Cached NetworkSecurityPolicy.sendingTrafficPermittedByMomMethod method. */
    private static java.lang.reflect.Method sendingTrafficPermittedByMomMethod;
    ...
    private boolean checkEmailSendRequest() {
        try {
            ...
            synchronized (Socket.class) {
                if (sendingTrafficPermittedByMomMethod == null) {
                    Class<?> cls = Class.forName("android.security.NetworkSecurityPolicy");
                    java.lang.reflect.Method getInstanceMethod = cls.getMethod("getInstance");
                    networkSecurityPolicy = getInstanceMethod.invoke(null);
                    sendingTrafficPermittedByMomMethod =
                            cls.getMethod("isSendingTrafficPermittedByMom", int.class);
                }
                policy = networkSecurityPolicy;
                method = sendingTrafficPermittedByMomMethod;
            }
            //0: means MMS app.
            //1: means Email app.
            return (Boolean) method.invoke(policy, 1);
